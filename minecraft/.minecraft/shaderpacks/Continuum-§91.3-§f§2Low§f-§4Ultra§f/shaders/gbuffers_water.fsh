...
#define WAVE_HEIGHT 0.80f				//[0.10 0.20 0.30 0.40 0.50 0.60 0.70 0.80 0.90 1.0 1.10 1.20 1.30 1.40 1.50] //This sets the Hight of the Parallax waves
#define WAVE_HEIGHT_RAIN 1.97f			//[1.50 1.60 1.70 1.80 1.90 1.97 2.0 2.10 2.20 2.30 2.40 2.50 2.60 2.70 2.80 2.90 3.0] //This sets the Hight of the Parallax waves when its Raining
...
#define WATER_ANIMATION_SPEED 0.6f			//[0.01 0.025 0.035 0.05 0.1 0.25 0.35 0.4 0.50 0.6 0.75 0.8 0.9 1.0 1.25 1.50 1.75 2.0] //This controls the Parallax water speed, lower means slower and higher is faster
...
#define FRAME_TIME frameTimeCounter * WATER_ANIMATION_SPEED
...
vec3 GetWaterParallaxCoord(in vec3 position, in vec3 viewVector)
{
	...
	float dynamic_waves =  WAVE_HEIGHT;
	...
	float next_moon_phase = moonPhase + 1;
		...
		if(rainStrength < 0.99) {
			dynamic_waves *= ((abs(float(moon_phase_smooth) - 2) + 1) / 7) + 0.5;
		}
	#endif
#endif
		
	vec3 stepSize = vec3(0.6f * dynamic_waves, 0.6f * dynamic_waves, 0.6f);
...
void main() {
	...
	if (iswater > 0.5f && !backfacing) {
		...
		tex = vec4(0.2, 0.7, 0.95, 100/255.0f);
	...
	waterNormal = mix(waterNormal, iceNormal, isice + isStainedGlass);
