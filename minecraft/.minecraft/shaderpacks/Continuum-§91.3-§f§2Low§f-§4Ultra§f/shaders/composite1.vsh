...
#define NIGHT_LIGHT 0.2			// [0.00015 0.0015 0.015 0.15 0.2 0.25 0.3 0.5 0.75 1.0]
...
void main() {
	...
	float time = float(worldTime);
	 night = clamp((time-12000.0)/300.0,0.0,1.0)-clamp((time-23000.0)/200.0,0.0,1.0);
	 day = clamp((time-22000.0)/300.0,0.0,1.0)-clamp((time-13000.0)/200.0,0.0,1.0);
	...
	transition_fading = 1.0-(clamp((timefract-12000.0)/300.0,0.0,1.0)-clamp((timefract-13000.0)/300.0,0.0,1.0) + clamp((timefract-23000.0)/200.0,0.0,1.0)-clamp((timefract-23200.0)/200.0,0.0,1.0));
	transition_fadingUp = 1.0+(((clamp((timefract),24000,12000.0) - 24000) / 12000) + ((clamp((timefract),12000,15000.0) - 12000) / 12000));	//the 15850.0 is the amount it increases higher number means more 
	transition_fadingDown = 1.0-(((clamp((timefract),24000,12000.0) - 24000) / 12000) + ((clamp((timefract),12000,15000.0) - 12000) / 12000));
