...
//#define ENABLE_SSAO				//Sonic Ethers AO
//#define ENABLE_HBAO				//Continuum's new AO (By Joey)

#define Global_Illumination

#define GI_QUALITY 0.5 				// Number of GI samples. More samples=smoother GI. High performance impact! [0.5 1.0 2.0 3.0]
...
#define GI_RADIUS 0.5 				// How far indirect light can spread. Can help to reduce artifacts with low GI samples. [0.5 0.75 1.0]

#define GI_Brightness 1000			//[100 200 300 400 500 600 700 800 900 1000 1100 1200 1300 1400 1500 1600 1700 1800 1900 2000] //This will increase the GI brightness and Bounce
...
float calculateHBAO(in vec2 coord) {
	...
	vec3 viewPosition = GetScreenSpacePosition(coord).xyz;
	vec3 viewNormal = GetNormals(coord);

	float total = 0.0;
	float sampleDirectionIncrement = 2.0 * PI / float(samplingDirections);

	vec2 randomRotation = CalculateNoisePattern1(vec2(0.0), 3).xy;
	...
	for(int i = 0; i < samplingDirections; i++) {
		...
		for(int j = 0; j < numSampleSteps; j++) {
			...
			vec3 offsetViewPosition = GetScreenSpacePosition(offset).xyz;
...
vec4 GetLight(in int LOD, in vec2 offset, in float range, in float quality, vec3 noisePattern)
{
		...
		&&  texcoord.t - offset.t + padding > 0.0f) 
	{
		...
		#ifdef ENABLE_HBAO
			if (!isSky) ao *= calculateHBAO(coord.st);
		#endif
